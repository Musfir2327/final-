    ...
    if (selectedDate) {
      const filtered = invoices.filter((invoice) => {
        const invoiceDate = invoice.date
          ? new Date(invoice.date).toLocaleDateString("en-CA") // Format as YYYY-MM-DD
          : "";
            ...
            <table style={tableStyle}>
              ...
              <tbody>
                {filteredInvoices.map((invoice) => (
                  <tr key={invoice.id}>
                    <td style={tableCellStyle}>{invoice.date ? new Date(invoice.date).toLocaleDateString() : "N/A"}</td>
