...
const CreateAppointment = () => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('http://localhost:8085/sa/Appointment/create', formData)
      .then((response) => {
        console.log('Appointment created:', response.data);
        alert("Appoiment Created succesfully ")
        // Navigate to AllAppointments page after the appointment is created
        // navigate('/send',{ state: { appointment: response.data } }); // Pass the response data as state
      })
      .catch((error) => {
  ...
  const handleCancel = () => {
    ...
    navigate('/patient-portal'); // Navigate back to home page or previous page
  };
